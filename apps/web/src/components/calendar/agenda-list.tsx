...
import { dotClasses, hrefForItem, isEditableEvent } from "@/components/calendar/calendar-item";
...
export function AgendaList({ items, onEditEvent }: { items: CalendarItem[]; onEditEvent: (item: CalendarItem) => void }) {
            ...
            {monthItems.map((item) => (
              <li key={item.id} className="flex items-center gap-3 py-2.5 text-[15px]">
                ...
                {isEditableEvent(item) ? (
                  <button type="button" onClick={() => onEditEvent(item)} className="min-w-0 flex-1 truncate text-left transition-colors hover:text-coral">
                    {item.title}
                  </button>
                ) : (
