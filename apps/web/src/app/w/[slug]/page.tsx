      ...
      <section>
        ...
        {destinations.length === 0 ? (
          ...
          <div className="-mx-4 mt-5 flex snap-x gap-4 overflow-x-auto px-4 pb-3 sm:-mx-8 sm:px-8">
            {[...destinations]
              .sort((a, b) => rankOf(a, scenarioByDestination) - rankOf(b, scenarioByDestination))
              .map((d, i) => (
                <DestinationPostcard
                  key={d.id}
                  destination={d}
                  scenario={scenarioByDestination.get(d.id)}
                  pinned={pinned?.destinationId === d.id}
...
function rankOf(d: Destination, scenarios: Map<string, Scenario>): number {
  const s = scenarios.get(d.id);
  if (s?.pinned) return -1;
  return s ? scenarioMath(s).totalCost : Number.MAX_SAFE_INTEGER;
}
